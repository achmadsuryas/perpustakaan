...
@section('container')
<div class="mb-5">
    <a href="{{ route('cv-pustakawan') }}">
        <button class="bg-red-700 p-3 text-white">
            Create Pustakawan <i class="fa-solid fa-plus fa-fw"></i>
        </button>
    </a>
</div>
                ...
                @foreach ($pustakawans as $pustakawan)
                    <tr
                        ...
                        <td class="px-6 py-4 flex">
                            <a href="{{ route('e-pustakawan', ['id' => $pustakawan->f_id]) }}">
                                <button type="submit">
                                    <p class="bg-blue-600 text-white py-1 px-3 font-semibold mr-2">EDIT</p>
                                </button>
                            </a>
                            <form action="{{ route('d-pustakawan', ['id' => $pustakawan->f_id]) }}" method="POST">
                                @method('delete')
                                @csrf
